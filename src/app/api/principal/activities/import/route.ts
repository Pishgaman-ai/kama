...
interface ImportRow {
  ...
  "عنوان فعالیت": string;
  "تاریخ": string;
  "تاریخ شمسی (مثال: 1403/10/15)": string;
  "نمره": number | string;
  ...
  try {
    ...
    try {
      ...
      teachersResult.rows.forEach((teacher) => {
        teachersMap.set(teacher.name, teacher);
      });
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          ...
          if (
            ...
            !row["درس"] ||
            !row["نوع فعالیت"] ||
            (!row["تاریخ"] && !row["تاریخ شمسی (مثال: 1403/10/15)"])
          ) {
            errors.push(
          ...
          // Find teacher from teacher_assignments
          const teacherAssignment = await client.query(
            `SELECT teacher_id
             ...
             LIMIT 1`,
            [student.class_id, lesson.id]
          );

          if (teacherAssignment.rows.length === 0) {
            errors.push(
              `ردیف ${rowNum}: هیچ معلمی برای درس '${row["درس"]}' در کلاس '${row["کلاس"]}' تخصیص داده نشده است`
            );
            continue;
          }
          ...
          // Support both old format "تاریخ" and new format "تاریخ شمسی (مثال: 1403/10/15)"
          const persianDateStr = (row["تاریخ شمسی (مثال: 1403/10/15)"] || row["تاریخ"])?.toString();
          if (!persianDateStr) {
            errors.push(
          ...
          // Support both old format "نمره کمی (0-20)" and new format "نمره"
          const scoreValue = row["نمره"] || row["نمره کمی (0-20)"];
          if (scoreValue !== null && scoreValue !== undefined && scoreValue !== "") {
            const score = Number(scoreValue);
            if (isNaN(score) || score < 0 || score > 20) {
              errors.push(
                `ردیف ${rowNum}: نمره باید عددی بین 0 تا 20 باشد`
              );
              continue;
            }
          ...
          const existingActivity = await client.query(
            `SELECT id FROM educational_activities
             WHERE student_id = $1
               ...
               AND teacher_id = $4
             LIMIT 1`,
            [student.id, dateStr, activityType, teacherId]
          );
