...
// GET endpoint to retrieve assessment results for a student in a specific subject
export async function GET(request: NextRequest) {
  try {
    ...
    const lifeSkillsQuery = `
      ...
      WHERE student_id = $1 AND subject_id = $2 AND class_id = $3
      ORDER BY assessment_date DESC
    `;
    const lifeSkillsResult = await pool.query(lifeSkillsQuery, [
      studentId,
      subjectId,
      classId,
    ]);
    ...
    const activeLifeQuery = `
      ...
      ORDER BY assessment_date DESC
    `;
    const activeLifeResult = await pool.query(activeLifeQuery, [
      studentId,
      subjectId,
      classId,
    ]);
    ...
    const growthDevQuery = `
      ...
      ORDER BY assessment_date DESC
    `;
    const growthDevResult = await pool.query(growthDevQuery, [
      studentId,
      subjectId,
      classId,
    ]);
...
// POST endpoint to create/update assessment results
