  ...
  try {
    ...
    if (identifier.includes("@")) {
      ...
      result = await client.query(
        "SELECT id, email, phone, password_hash, name, national_id, role, school_id, profile_picture_url, profile, created_at FROM users WHERE email = $1",
        [identifier]
      );
    } else if (/^09\d{9}$/.test(identifier)) {
      // If matches Iranian phone format, treat as phone
      result = await client.query(
        "SELECT id, email, phone, password_hash, name, national_id, role, school_id, profile_picture_url, profile, created_at FROM users WHERE phone = $1",
        [identifier]
      );
    } else {
      // Otherwise, treat as national_id
      result = await client.query(
        "SELECT id, email, phone, password_hash, name, national_id, role, school_id, profile_picture_url, profile, created_at FROM users WHERE national_id = $1",
        [identifier]
      );
    }
...
export async function getUserById(id: string): Promise<User | null> {
  try {
    ...
    const result = await client.query(
      "SELECT id, email, phone, name, national_id, role, school_id, profile_picture_url, profile, created_at FROM users WHERE id = $1",
      [id]
    );
    client.release();
...
export async function resetPasswordWithToken(
  ...
  try {
    ...
    const userResult = await client.query(
      "UPDATE users SET password_hash = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING id, email, name, role, profile_picture_url, created_at",
...
export async function sendOTPToPhone(
  ...
  try {
    ...
    // Send OTP via SMS with optional template
    const smsResult = await sendOTP(normalizedPhone, otpCode, templateName);
...
export async function verifyOTPAndSignIn(
  ...
  try {
    ...
    // Get user by phone
    const userResult = await client.query(
      "SELECT id, email, phone, name, national_id, role, school_id, profile_picture_url, created_at FROM users WHERE phone = $1",
      [normalizedPhone]
    );
...
export async function signInWithNationalID(
  ...
  try {
    ...
    const result = await client.query(
      "SELECT id, email, phone, password_hash, name, national_id, role, school_id, profile_picture_url, created_at FROM users WHERE national_id = $1",
      [nationalId]
    );
