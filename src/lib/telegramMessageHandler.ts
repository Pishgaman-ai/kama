...
// Business logic for processing Telegram messages and interfacing with AI

import pool from "./database";
import { sendMessage, sendTypingAction, splitLongMessage, delay } from "./telegramService";
...
/**
 * Identify user by their Telegram chat ID
 * @param telegramChatId The Telegram chat ID (from message sender)
 * @returns User object if found, null otherwise
 */
export async function identifyUserByTelegramChat(telegramChatId: string): Promise<User | null> {
  try {
    const result = await pool.query(
      `SELECT id, email, phone, name, national_id, role, school_id,
              profile_picture_url, profile, is_active, created_at
       FROM users
       WHERE profile->>'telegram_chat_id' = $1
         AND is_active = true
       LIMIT 1`,
      [telegramChatId]
    );
    ...
    console.error("Failed to identify user by Telegram chat ID:", error);
    return null;
  }
}
...
export async function getBotTokenForSchool(schoolId: string): Promise<string | null> {
  try {
    const result = await pool.query(
      `SELECT profile->>'telegram_api_key' as bot_token
       FROM users
       WHERE school_id = $1
         AND role = 'principal'
         AND is_active = true
    ...
    if (result.rows.length === 0 || !result.rows[0].bot_token) {
      ...
      if (envToken) {
        console.warn(
          `Using TELEGRAM_BOT_TOKEN from environment for school ${schoolId} (principal profile token missing)`
        );
        return envToken;
      }
      return null;
...
/**
 * Send AI response to Telegram (handles streaming and chunking)
 * @param botToken The Telegram bot API token
 * @param chatId The Telegram chat ID to send response to
 * @param aiStream The ReadableStream from AI
 */
export async function sendAIResponseToTelegram(
  botToken: string,
  ...
  try {
    ...
    for (const msg of messages) {
      ...
      if (!response.ok) {
        console.error(
          `Failed to send message chunk to Telegram (chat ${chatId}):`,
...
/**
 * Log unknown Telegram interaction for debugging
 * @param chatId The Telegram chat ID
 * @param messageText The message text
 * @param error Optional error message
 */
export async function logUnknownTelegramInteraction(
  chatId: string | number,
  messageText: string,
  error?: string
): Promise<void> {
  try {
    console.warn("Unknown Telegram interaction:", {
    ...
    console.error("Failed to log unknown Telegram interaction:", err);
  }
}
